    ...
    <>
      <div className="bg-primary-900 flex rounded-xl flex-wrap sm:justify-center items-center xl:justify-between text-center px-4 py-2 text-primary-300 text-xl w-full">
        <div className="flex w-[500px] m-2">
          <h1 className="font-bold justify-start text-center lg:text-right leading-none overflow-hidden whitespace-nowrap text-ellipsis w-[99%]">
            {event?.description}
          </h1>
        </div>
        <div className="flex flex-auto m-2 sm:justify-center xl:justify-end">
          <DropDown
            Title="لینک رویداد"
            Icon={<AiOutlineLink className="ml-1" />}
            Type="primary"
          >
            {event?.link && (
              <div
                className="cursor-pointer p-2 border mt-2 border-primary-800  bg-primary-1000 rounded-lg flex items-center"
                onClick={() => handleCopyToClipBoard(event?.link)}
              >
          ...
          {auth.role == Roles.HOST && (
            <Button
              className="mx-2"
              variant="danger"
              icon={<IoMdExit className="ml-2" />}
              title="اتمام رویداد"
              onClick={() => setIsOpenEndShow(true)}
            />
          )}
        </div>
      </div>
      {auth.role == Roles.HOST && (
        <Modal
          title="اتمام رویداد"
          setIsOpen={setIsOpenEndShow}
          isOpen={isOpenEndShow}
        >
          <div className="flex flex-col items-center text-primary-400">
            ...
            <h3 className="text-sm mt-2">
              رویداد برای همه شرکت کنندگان قطع می شود.
            </h3>
            <div className="flex [&_button]:mt-4 [&_button]:mx-3">
              <Button
                ...
                title="انصراف"
                outlined
                onClick={() => setIsOpenEndShow(false)}
              />
              <Button
                variant="danger"
                title="اتمام رویداد"
      ...
      <Drawer
        ...
        description={
          vote.needResetVote
            ? "نظرسنجی خود را ایجاد کنید تا کاربران بتوانند در نظر سنجی پخش زنده شرکت کنند"
            : ""
        }
        title={vote.needResetVote ? "تعریف نظرسنجی" : "سوال نظرسنجی"}
      >
        <Poll />
      </Drawer>
    </>
