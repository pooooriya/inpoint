...
const { btn, btn_icon, btn_disabled, btn_primary, btn_primary_outline, btn_secondary_outline, btn_danger_outline, btn_danger, btn_secondary }: Record<string, string> = {
    ...
    btn_secondary: "bg-secondary text-primary-200 rounded-md py-3 text-center",
...
export const Button = ({ variant, title, icon, loading, className, outlined, disabled, ...props }: ButtonProps) => {
    ...
    switch (variant) {
        case "primary":
            return (
                <button className={cs(btn, btn_primary, outlined && btn_primary_outline, className)} {...props} >
        ...
        case "danger":
            return (<button className={cs(btn, btn_danger, outlined && btn_danger_outline, className)} {...props} >
                <div className="flex justify-centers items-center">
                    {loading ? <LoadingButton className='ml-2' /> : icon}
                    {title}
                </div>
        ...
        case "secondary":
            return (<button className={cs(btn, btn_secondary, outlined && btn_secondary_outline, className)} {...props} >
                <div className="flex justify-centers items-center">
                    {loading ? <LoadingButton className='ml-2' /> : icon}
                    {title}
                </div>
